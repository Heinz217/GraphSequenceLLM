...
class GslMetaModel:
    ...
    def __init__(self, config):
        super(GslMetaModel, self).__init__(config)
    ...
    def initialize_graph_modules(self, model_args, fsdp=None):
        ...
        """
        这里对config的mm_projector_type和mm_hidden_size进行了定义
        """
...
class GslMetaForCausalLM(ABC):
    ...
    def encode_graphs(self, graph, graph_emb):
        graph_features = self.get_model().mm_projector(graph_emb)  # 先将全图进行投影
        graph_features[graph==DEFAULT_GRAPH_PAD_ID] = 0.  # 之后在将pad取0
        return graph_features

    def inject_special_token(self, graph_emb):
        """
        这里还是不太明白special token的作用
        :param graph_emb:
        :return:
        """
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        graph_features = self.encode_graphs(graphs, graph_emb)  # 得到图特征
